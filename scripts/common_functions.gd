...
func _on_player_data_recived():
	print("player_data_recived")
	match no_of_players:
		2:
			var temp: int = randi_range(0,2)
			for player in players_list:
				player.position = tilemap_reference.get_cell_atlas_coords(0, spawn_points[temp])
				temp += 3
		3:
		...
		4:
			var temp: int = randi_range(0,1)
			for i in range(0,1):
				players_list[i].position = tilemap_reference.get_cell_atlas_coords(0, spawn_points[temp])
				players_list[i + 3].position = tilemap_reference.get_cell_atlas_coords(0, spawn_points[temp+3])
				temp += 3
		5:
			...
			for i in range(0,5):
				if i == temp:
					continue
				players_list[i].position = tilemap_reference.get_cell_atlas_coords(0, spawn_points[i])
		6:
			for i in range(0,5):
				players_list[i].position = tilemap_reference.get_cell_atlas_coords(0, spawn_points[i])
